...
https://www.xiaoran.de/posts/installqbt
...
```shell
nohup /root/chfs/chfs --file=/root/chfs/chfs.ini &
```

> [参考链接](https://blog.51cto.com/zounan/4932317)
...
- 重新加载bash：`source ~/.bashrc`

- 开启自动更新：`acme.sh --upgrade --auto-upgrade`

- 切换到 Let‘s Encrypt：`acme.sh --set-default-ca --server letsencrypt`

- 使用DNS验证签发证书

  - 参考教程：https://u.sb/acme-sh-ssl/

- 安装证书

  - ```shell
    acme.sh --install-cert -d 120001.xyz \
    --key-file       /root/cert/120001.xyz.key  \
    --fullchain-file /root/cert/120001.xyz.crt \
    --ca-file        /root/cert/120001.xyz.crt \
    ```
