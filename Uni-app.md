...
- 转换为小程序时：组件外面会用`view`包裹，所以类名不生效，可以用上层类名
  ...
  ![image-20230628133228317](./assets/Uni-app/image-20230628133228317.png)
...
- **重要：**uni-app中不管在`data`中是否有定义，都可以直接`this.test = 'abc'`进行赋值而不会报错
  ...
  ![image-20230628133233208](./assets/Uni-app/image-20230628133233208.png)
...
- 若要**绑定样式**，可使用计算属性，直接绑定不生效（可能原因：渲染DOM的时候未拿到值，渲染不出来）
  ...
  - ![image-20230628133237720](./assets/Uni-app/image-20230628133237720.png)

  - 解决方法一：使用计算属性

    ![image-20230628133245719](./assets/Uni-app/image-20230628133245719.png)

  - 解决方法二：使用数组写法（colorUI案例）

    ![image-20230628133249419](./assets/Uni-app/image-20230628133249419.png)
